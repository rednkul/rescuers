...
os.environ.setdefault('DJANGO_SETTINGS_MODULE', 'rescuers.rescuers.settings')
import django
django.setup()
...
month_names = ['января', 'февраля', 'марта', 'апреля', 'мая', 'июня', 'июля', 'августа', 'сентября', 'октября',
                   'ноября', 'декабря']
excel_file = openpyxl.load_workbook('список.xlsx')
...
class ListImportToExcel(State):
    ...
    def make_excel(self):
        ...
        for division in Division.objects.all():
            ...
            excel_sheet[f'A{self.row_number}'] = f'{division.name} пидар'
            ...
            for index, worker in enumerate(division.division_workers.all().order_by('post__priority')):
                print(f'{index + 1}.{worker.get_full_name()}')
        ...
        self.make_total()
        excel_file.save(filename='отчеты 2/список1.xlsx')
    ...
    def make_total(self):
        ...
        excel_sheet[f'G{row_number}'] = Worker.objects.filter(post__name="Командир отряда").count()
        ...
        excel_sheet[f'G{row_number}'] = Worker.objects.filter(post__name="Командир отделения").count()

        row_number += 1

        excel_sheet[f'F{row_number}'] = 'респираторщик'
        excel_sheet[f'G{row_number}'] = Worker.objects.filter(post__name="Респираторщик").count()
